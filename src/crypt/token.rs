...
fn _generate_token(ident: &str, duration_sec: f64, salt: &str, key: &[u8]) -> Result<Token> {
  ...
  let sign_b64u = _token_sign_into_b64(&ident, &exp, salt, key)?;
...
fn _validate_token_sign_and_exp(origin_token: &Token, salt: &str, key: &[u8]) -> Result<()> {
  // -- Validate signature.
  let new_sign_b64u = _token_sign_into_b64(&origin_token.ident, &origin_token.exp, salt, key)?;
...
/// and salt.
fn _token_sign_into_b64(ident: &str, exp: &str, salt: &str, key: &[u8]) -> Result<String> {
...
mod tests {
  ...
  fn test_token_display_ok() -> Result<()> {
    // -- Fixture
    let fx_token_str = "ZngtaWRlbnQtMDE.MjAyNC0wNi0yN1QxMToyMDowMFo.some-sign-b64u-encoded";
    let fx_token = Token {
      ident: "fx-ident-01".to_string(),
      exp: "2024-06-27T11:20:00Z".to_string(),
